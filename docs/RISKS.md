...
| R-016 | Le garde-fou Bash **n'est pas une frontière de sécurité** : 8 formulations sur 11 le contournaient avant correction. Un obfuscateur déterminé passera toujours | moyenne | haute | 60 cas de test dont les 10 contournements de l'audit ; limite écrite dans l'en-tête du hook ; la liste `deny` ne double plus le hook sur les commandes (elle ne couvre que la lecture de fichiers) | à désigner | **accepté** |

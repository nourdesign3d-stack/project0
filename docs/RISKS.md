...
| R-015 | `trustPolicy: no-downgrade` a écarté quatre paquets en une semaine (@langchain/core, @arcjet/next, resend, chokidar) : il refuse aussi des correctifs de sécurité légitimes. `pnpm dedupe` reste inutilisable sur cet arbre | moyenne | avérée | exceptions nommées et datées dans `pnpm-workspace.yaml` (`trustPolicyExclude`), une par paquet, avec sa raison | à désigner | ouvert |

...
| R-014 | 88 alertes de vulnérabilité ouvertes après traitement (252 fermées par les montées de Next) : 30 sur `hono`, 12 sur `undici`, le reste transitif | moyenne | avérée | règle de tri dans `DEPLOYMENT.md` ; passe mensuelle avec les PR Dependabot | à désigner | ouvert |
| R-015 | Entrée orpheline `next@16.1.6` dans le lockfile : rien ne la référence, mais GitHub la lit et maintient 28 alertes fantômes. `pnpm dedupe` la purgerait, mais échoue sur `trustPolicy: no-downgrade` — `@arcjet/next@1.2.0` présente une régression de preuve de publication | faible (bruit) mais masque de vraies alertes | avérée | aucun ; trois options ouvertes : garder le contrôle et vivre avec le bruit, examiner Arcjet, ou retirer Arcjet (inactif faute de clé — R-003) | propriétaire du dépôt | ouvert |
